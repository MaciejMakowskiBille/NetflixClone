...
import {
  getAllTypeMoviesByDate,
  getAllTypeMoviesByProducer,
  getAllTypeMoviesBySearch,
  getCategories,
  getFilms,
  getSeries,
} from "../../utils/Gets";
...
const FilteredMovies = () => {
  ...
  const [filteredData, setFilteredData] = useState<CombinedDataType | null>(
    null
  );
        ...
        <main>
          <div className="filterPage">
            {type && type === "producers" && filter && (
              <div className="producerTitle">
                Produkcje od: <span>{filter}</span>{" "}
              </div>
            )}
            {type && type === "search" && filter && (
              <div className="producerTitle">
                Wyszukane dla: <span>{filter}</span>{" "}
              </div>
            )}
            {type && type === "new" && (
