...
const MoviePage = () => {
    const [movieData, setMovieData] = useState<
        MovieDataType | SeriesDataType | null
    >(null);
    const [isLoading, setIsLoading] = useState(true);
    const [favButtonClass, setFavButtonClass] = useState<
        'activeFavButton' | 'favButton'
    >('favButton');
    const favoriteMovies = useFavoriteMoviesIds({
        reloadValueCheck: favButtonClass,
    });
    ...
    const showLength = () => {
        ...
        if (movieData && 'seasons' in movieData) {
            return <>Sezony: {movieData.seasons.length}</>;
        }
    };
    ...
    useEffect(() => {
        if (movieType === 'm' && favoriteMovies?.includes(Number(movieId))) {
            ...
            movieType === 's' &&
            favoriteSeries?.includes(Number(movieId))
        ) {
            setFavButtonClass('activeFavButton');
        } else {
            setFavButtonClass('favButton');
                    ...
                    <main>
                        <div className='movieInfoMain'>
                            <div
                                ...
                                style={{
                                    backgroundImage: `url(${
                                        serverURL + movieData.primaryImg
                                    })`,
                                }}
                            />
                            <div className='content'>
                                ...
                                {movieType === 'm' ? (
                                    <div className='buttons'>
                                        ...
                                        <button
                                            className={`iconButton icon ${favButtonClass}`}
                                    ...
                                    <div className='buttons'>
                                        <button
                                            className={`iconButton icon ${favButtonClass}`}
                                            onClick={handleAddSeriesToFavorites}
                                        />
                                    </div>
                                )}
                        ...
                        <MoviePageNav active={active} setActive={setActive} />
                        <MoviePageAdds
                            id={movieData.id}
                            active={active}
                            ...
                            {...('director' in movieData && {
                                director: movieData.director,
                            })}
                            {...('duration' in movieData && {
                                duration: movieData.duration,
                            })}
                            {...('seasons' in movieData && {
