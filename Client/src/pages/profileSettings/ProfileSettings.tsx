...
import AddProfileModal from '../selectProfilPage/components/addPorfileModal';
...
const ProfileSettings = () => {
    ...
    const [removeProfileMessage, setRemoveProfileMessage] =
        useState<string>('');
    ...
    async function loadUserData() {
        await getAllUserData().then((response) => {
            ...
            if (response?.payment?.paymentsOffer) {
                setPaymentsOfferText(
                    '<p>Plan miesięczny</p><p>29.99zł/miesiąc</p>'
                );
            } else {
    ...
    const onSubmit = async (formData: SettingsSchemaType) => {
        ...
        try {
            if (key === 'currentPassword' || key === 'password') {
                ...
                changePasswordData['passwordConfirmation'] =
