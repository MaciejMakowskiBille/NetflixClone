...
export const getEpisode = async (id: number): Promise<Episode | null> => {
    const response = await axios.get(apiURL + `episodes/${id}?populate=deep`);
    if (response && response.data.data) {
        return clearEpisodeData(response.data.data);
    } else {
        return null;
    }
};
