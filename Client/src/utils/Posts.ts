...
export async function signIn(data: SignInType) {
    try {
        const response = await axios.post(authURL + 'local', {
...
export function postPayment(endpoint: string, data: PaymentsType) {
    const response = axios
        ...
        .then((response) => response.data.data as PaymentsResponseType)
        .catch((err) => {
...
export const uploadImage = async (image: File): Promise<number | null> => {
    const formData = new FormData();
    formData.append('files', image);
    let imageId;
    await axios
        .post(uploadURL, formData)
        .then((response) => (imageId = response.data[0].id))
        .catch((error) => {
            throw new Error(
                'Wystąpił błąd podczas wysyłania zasobów:\n' + error
            );
        });
    if (imageId) {
        return imageId;
